...
[CreateAssetMenu(fileName = "MindRuneEvent", menuName = "BodyMind/Events/Mind/Mind Rune")]
public class MindRuneEvent : MapEvent
{
    private PlayerData Player => GameManager.Instance.Player;
    private RuneData MindRune => GameManager.Instance.Runes.Mind;
    private DialogController Dialog => DialogController.Instance;

    public override bool OnInteract()
    {
        if (!Player.Runes.HasRune(MindRune))
        {
    ...
    {
        MessageController.Display("You now know the power of the Mind rune.");
        Player.Runes.AddRune(MindRune);
    };
